    ...
    return (
        <div className="print-preview-backdrop fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="print-preview-content bg-white rounded-lg w-96 font-sans shadow-2xl" onClick={e => e.stopPropagation()}>
                <div id="printable-sticker-content" className="p-6 border-2 border-dashed border-gray-400 rounded-t-lg text-center bg-white">
                    <div className="flex items-center justify-center space-x-2 mb-4">
                        <LogoIcon className="h-8 w-auto text-primary" />
                        <span className="text-2xl font-fredoka text-fun-orange">Bravos!</span>
                    </div>
                    <p className="text-xl font-semibold text-dark-text mb-4">Scan to send a Bravo!</p>
                    <div className="flex justify-center p-2 bg-white rounded-lg border border-gray-200 inline-block">
                        <QRCode value={qrUrl} size={200} />
                    </div>
                    <div className="mt-4">
                        <h3 className="text-3xl font-bold text-dark-text">{associate.name}</h3>
                        <p className="text-lg text-medium-text">{associate.role}</p>
                        {associate.aboutMe && <p className="text-sm text-medium-text mt-2 italic">"{associate.aboutMe}"</p>}
                        {corporationName && (
                             <p className="text-base font-semibold text-indigo-600 mt-1">{corporationName}</p>
                        )}
                    </div>
                </div>
...
const ProfileEditor: React.FC<{ associate: Associate }> = ({ associate }) => {
    ...
    const handleSave = async () => {
        if (!hasChanges || !db || !associate.authUid) return;
        ...
        try {
            ...
            const updatedSettings = { name, aboutMe, avatarUrl };
            
            await db.collection('associates').doc(associate.id).update(updatedSettings);
            await db.collection('users').doc(associate.authUid).update({ avatarUrl });
    ...
    const inputStyles = "mt-1 block w-full p-2 bg-white border border-gray-300 rounded-md text-dark-text placeholder-gray-400 focus:outline-none focus:ring-primary focus:border-primary";
