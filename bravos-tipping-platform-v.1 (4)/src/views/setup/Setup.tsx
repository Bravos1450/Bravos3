...
const Setup = () => {
    ...
    const handleEmployeeCheck = async () => {
        ...
        try {
            ...
            const q = db.collection('associates').where('corporateId', '==', employerId).where('email', '==', email);
    ...
    const handleCreateFirebaseAccount = async () => {
        ...
        if (userType === 'individual') {
            ...
            await db.collection('associates').doc(foundAssociate.id).update({ authUid: authUser.uid, name, aboutMe, avatarUrl: foundAssociate.avatarUrl });
            ...
            const newCorpRef = db.collection('corporations').doc();
            
            await db.collection('users').doc(authUser.uid).set({ 
                type: 'corporate', 
                corporateId: newCorpRef.id, 
            ...
            if (avatarFile) {
                logoUrl = await uploadImage(avatarFile, `logos/${newCorpRef.id}/${avatarFile.name}`);
            }

            await newCorpRef.set({ name: companyName, allowTips: true, logoUrl: logoUrl });

            await db.collection('associates').add({
                name, role: 'Administrator', isCorporate: true, corporateId: newCorpRef.id,
                email, authUid: authUser.uid, avatarUrl: logoUrl,
            ...
            homePath = '/corporate';
             ...
             if (tipIdToClaim) {
                 await db.collection('tips').doc(tipIdToClaim).update({ customerAuthUid: authUser.uid });
